...
function Purchasereturn() {
  ...
  const columns = [
    ...
    {
      selector: (row) => row.purchasereturnDate,
      name: "Date",
      sort: true,
    },

    {
      selector: (row) => row.purchasereturnNumber,
  ...
  const getallcreditdata = async () => {
    try {
      const response = await axios.get(
        "http://localhost:9001/api/purchase/purchasereturns/getAllpurchasereturn"
      );
  ...
  const addSalesItem = async () => {
    try {
      let config = {
        url: "/purchase/purchasereturns/addpurchasereturn",
        method: "post",
  ...
  const deletequotation = async (data) => {
    try {
      const response = await axios.delete(
        `http://localhost:9001/api/purchase/purchasereturns/deletepurchasereturn/${data._id}`
      );
      if (response.status === 200) {
    ...
    <>
      ...
      {item && !ModifyITems && (
        <div className="container p-5">
          <p className="textbld f_20">New Purchase Return </p>
          <div className="row mt-5  ">
            <div className="col-md-8">
              <div className="row ">
                <div className="col-md-4">
                  <p className="colr-red textbld">Vendor *</p>
                </div>
              ...
              <div className="row ">
                <div className="col-md-4">
                  <p className="colr-red textbld">Debit Note Number*</p>
                </div>
                <div className="col-md-8 mb-4">
                  <Form.Control
                    placeholder="Debit Note Number"
                    value={purchasereturnNumber}
                    onChange={(e) => setpurchasereturnNumber(e.target.value)}
                  />
                </div>
              </div>
              <div className="row ">
                <div className="col-md-4">
                  <p className="colr-red textbld">Debit Note Date*</p>
                </div>
