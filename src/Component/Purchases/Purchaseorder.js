...
function Purchaseorder() {
  ...
  const [searchpurchaseorder, setsearchpurchaseorder] = useState("");

  const addpurchaseorder = async () => {
    try {
      let config = {
        url: "/purchase/adspurchasesales",
        method: "post",
  ...
  const getAllreturnsales = async () => {
    try {
      const response = await axios.get(
        "http://localhost:9001/api/purchase/getAllpurchaseorder"
      );
      if (response.status === 200) {
        // console.log("AllItems=====>", response.data);
        setallpurchasedata(response.data.purchaseorder);
      }
    } catch (error) {
      console.warn(error);
    }
  ...
  const filterdata = allpurchasedata.filter(
    (item) =>
      (item.purchaseorderDate &&
        item.purchaseorderDate
          .toLowerCase()
          .includes(searchpurchaseorder.toLowerCase())) ||
  ...
  const deletepurchaseorder = async (data) => {
    try {
      const res = await axios.delete(
        `http://localhost:9001/api/purchase/deletepurchaseorder/${data._id}`
      );
        ...
        <div className="container p-5">
          ...
          <div className="row mt-5  ">
            <div className="col-md-8">
              ...
              <div className="row ">
                <div className="col-md-4">
                  <p className="colr-red textbld">Purchase Order Number*</p>
                </div>
                <div className="col-md-8 mb-4">
                  <Form.Control
              ...
              <div className="row ">
                <div className="col-md-4">
                  <p className="colr-red textbld">Purchase Order Date*</p>
                </div>
