...
function Debit() {
  ...
  const columns = [
    ...
    {
      selector: (row) => row.debitnoteDate,
      name: "Date",
      sort: true,
    },

    {
      selector: (row) => row.debitnoteNumber,
  ...
  const getallcreditdata = async () => {
    try {
      const response = await axios.get(
        "http://localhost:9001/api/purchase/debitnote/getAlldebitnote"
      );
  ...
  const addSalesItem = async () => {
    try {
      let config = {
        url: "/purchase/debitnote/adddebitnote",
        method: "post",
  ...
  const deletequotation = async (data) => {
    try {
      const response = await axios.delete(
        `http://localhost:9001/api/purchase/debitnote/deletedebitnote/${data._id}`
      );
      if (response.status === 200) {
