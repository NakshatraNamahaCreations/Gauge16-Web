...
function InvoicePullformquotation() {
  ...
  const editSalesItem = async () => {
    try {
      let config = {
        url: `/transaction/salesorder/editinvoice/${quotations._id}`,
        method: "put",
  ...
  const [ModifySalesOrdernumber, setModifySalesOrdernumber] =
    useState("QS-00032");
  const [ModifyDeliveryMethod, setModifyDeliveryMethod] = useState("xyz");
  const [ModifyPaymentTerms, setModifyPaymentTerms] =
    useState("Due on Reciept");
  const [ModirfyAddress, setModirfyAddress] = useState("");
  const [EditSalePerson, setEditSalePerson] = useState("Rohan");
  const [ModifySalesOrderdate, setModifySalesOrderdate] =
    useState("11/27/2023");
      ...
      {!pullquotaion ? (
        <div className="container p-4">
          ...
          <div className="row mt-3 m-auto">
            <TextField
              className="mb-4"
              sx={{ width: "35ch", borderRadius: 2 }}
              size="small"
              id="outlined-start-adornment"
              placeholder="Search  for pending quotations"
              type="text"
              InputProps={{
                startAdornment: (
            ...
            <div className="col-md-4">
              <div className="row">
                ...
                <Button className="col-md-5 textbld m-auto m-2" variant="light">
                  <Link className="anchore_Tag bordernone" to="/Sales">
                    Cancel
                  </Link>
                </Button>{" "}
              </div>
...
export default InvoicePullformquotation;
